...
Finds the gap between what Anton DOES (recurring manual work across sessions) and what he HAS (installed skills), and proposes the highest value × frequency new skills. Read-only; proposes, never auto-builds. Mirror of evaluate-recurring-into-routine [internal] (that catches one task → routine; this scans the whole fleet of sessions → skill gaps).

## Method (token-cheap — cheap tools first, vault-data-architecture [internal])
1. **The week's agenda (≈0 tokens):** `mcp__ccd_session_mgmt__list_sessions` (limit ~60) — session TITLES are the agenda; cluster them by theme. Don't read every transcript; use titles + memory. (Full-text `search_session_transcripts` needs Anton's approval — only if a cluster is ambiguous.) ⚠️ This only sees THIS machine's sessions; with the cross-machine relay live (machine-migration [internal], skill `inbox`) a pattern from another computer is invisible here — note the current-machine label and don't assume the gap is global.
...
3. **Cross-map:** for each session cluster, is there a skill that covers it? RECALL memory (machine-migration [internal], n8n-stack [internal], notebooklm-integration [internal], etc.) so you don't propose something a neighboring session already built — this is the #1 trap (a parallel session, INCLUDING one on another machine, may have shipped it; check `/inbox` / the relay).
4. **Rank gaps by value × repeat:** how many sessions touched it? does it hurt now? is the infra already there (→ thin wrapper, cheap to build)? Flag ⚠️ УСЛОЖНЕНИЕ for anything heavy (ak47-simplicity [internal]).

## Output
- A ranked table: gap · откуда видно (which sessions) · оценка (HIGH/MED/LOW) · есть ли уже инфра.
- Top 2-3 as **ДО→ПОСЛЕ on real data** (show-before-after [internal]).
- Refresh the dashboard: `$OBSIDIAN_VAULT\_Dashboards\Skill-Gap-Audit.html` (Anton works by eye, prefer-visual-dashboards [internal]).
- Then 🧒 recap. End by asking which to build (his "+" = go).

## Daily routine
Scheduled twin `skill-gap-daily` runs this read-only and updates the dashboard + drops a one-line note if a NEW gap appeared since yesterday (don't nag if nothing changed). Grunt drafting → Sonnet; the judgment/ranking → keep on the session model (model-routing-sonnet-grunt [internal]).
