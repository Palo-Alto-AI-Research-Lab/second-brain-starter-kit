...
description: >
  ...
  готовыми движками. Канон: память crash-recovery-command [internal], turnstate-ledger [internal], claude-desktop-sessions-per-account [internal].
---
...
```bash
python "$IMPORTS_ROOT\turnstate\turnstate_show.py" --n 12
```
...
```
Затем 🧒 «Простыми словами» (память eli5-always [internal]).
...
## Канон
Память crash-recovery-command [internal]. Кирпичи: turnstate-ledger [internal] (чёрный ящик), claude-desktop-sessions-per-account [internal] (continue_session), system-architect [internal] (/arch), `syncthing-desktop-laptop-sync` (/sync-check), mcp-health-check [internal] (/mcp). Пара к SessionStart-хуку `session_resume_hook` (тот показывает прошлую сессию сам на старте — этот собирает recall+здоровье+полную историю по команде).
