...
description: >-
  ...
  (preserve-originals-rule [internal], vault-offsite-backup [internal]); this skill is the
...
```
(For an in-place vault you can also recover from the live repo: `git -C $OBSIDIAN_VAULT restore --source <hash> -- <path>` — see vault-backup-rule [internal].)
...
- **Drive not uploading** → confirm Google Drive for Desktop is running and signed into `[email]`; the local `Obsidian-Backup\` is on E: and syncs from there.
...
- The standing rules are in memory (preserve-originals-rule [internal], vault-offsite-backup [internal]); reference them, don't fork them.
