---
name: declined
description: Быстрый доступ к реестру ОТКЛОНЁННЫХ/отложенных решений Антона — «что мы уже отвергали и почему», чтобы не пере-предлагать одно и то же. Trigger on "/declined", "что мы отвергали", "это уже отклоняли?", "покажи отклонённые решения", "реестр отказов", "проверь declined", "we rejected this before?", "show declined decisions". READ-ONLY обзор; опционально гоняет детектор новых отказов. Канон: память declined-decisions [internal]; ночной скан = `declined_scan.py`.
---
...
RELATION (не дублировать): реестр-источник = память declined-decisions [internal]; ночной скан = `declined_scan.py`; правило-для-людей = Библия `reglament-otvergnutoe-reshenie-v-reestr-declined`.
