...
- **Память:** создать/обновить `memory\<slug>.md` с frontmatter (`name`, `description`, `metadata.type`: user/feedback/project/reference). Body: для feedback/project — `**Why:**` + `**How to apply:**`. Линки `name [internal]` либерально. + одна строка-указатель в `MEMORY.md` (`- [Title](file.md) — hook`, ≤200 симв).
- **CLAUDE.md:** если always-on — добавить блок `## ALWAYS: <тема> (standing — set <дата>)` или дописать буллет в существующий; закончить «Канон: память `<slug>` + Библия `<reglament>`». ⚠️ Бюджет входа (write-service-files-tight-no-recompress [internal]): блок = ТОЛЬКО триггер + суть + указатель, цель ≤4 строк / ~900 символов; механика (команды, форматы, ID, примеры) живёт в Библии/памяти/скилле, в CLAUDE.md ей не место. После записи прогнать `python ~/.claude/scripts/claude_md_guard.py` — RED (>82 KB) чинится сразу структурным сжатием, не откладывается.
...
- Запись в волт → СНАЧАЛА `vault_backup.py` (vault-backup-rule [internal]).
- **⚠️ Если общий бэкап заблокирован** (предохранитель ловит чужие массовые удаления параллельного флота) — НЕ форсить; коммитить ТОЛЬКО свои файлы: `cd E:/Obsidian/Anton-Knowledge && git add "<мой файл>" && git commit -m "..."`. Никогда `--force`, никогда blanket-glob-delete.
...
Коротко Антону: **ЧТО** за правило · **КУДА** разложил (память/CLAUDE.md/Библия/MOC/hook/task) · **что перелинковал** · конфликты/дубли (если обновил существующее). Как с концептами — постфактум, не спрашивая разрешения на каждый дом.
