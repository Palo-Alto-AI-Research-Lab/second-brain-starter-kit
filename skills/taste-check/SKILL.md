---
name: taste-check
description: "Авто-ревьюер КАЧЕСТВА КОНТЕНТА (заметки волта / черновики исходящего / дедуп-мерджи / служебные файлы) — явный вердикт ✅ pass / ❌ fail / ⚠️ manual-review ДО показа Антону. Как /tt, но для контента: /tt проверяет «работает ли то, что собрали», /taste-check — «не стыдно ли показать/отправить то, что написали». Trigger on \"/taste-check <путь|текст>\", \"/taste\", \"проверь вкус\", \"проверь заметку перед показом\", \"taste check\", \"прогони через вкус\", а также САМ перед выдачей Антону свежесозданной заметки/черновика/мерджа. Первый фильтр, не финальный судья (судья = Антон). READ-ONLY: ревьюер НЕ пишет в волт и НЕ правит контент — только вердикт + список нарушенных принципов. Образец: taste-skill Артура Арсёнова (сигнал → принцип → проверка; гейт = рубильник с владельцем). Принципы выведены ТОЛЬКО из реальных сигналов Антона (rule_scan digests + memory с цитатами), не выдуманы."
---
...
- **P4 ⛔ (все) Секретов нет.** Сигнал: пароли живут в `secrets\`, не в волте/always-loaded (credential-store: реальные креды из чата немедленно уводим в store). Проверка: grep `парол|password|2fa|api[_-]?key|token|secret[_-]?key` + похожие на кред строки → hit по реальному секрету = FAIL.
...
- **P7 ⚠️ (A,C) Wikilinks резолвятся.** Сигнал: «0 broken links перед staging→vault» (vault-conventions). Проверка: каждый `target [internal]` существует как файл/alias; битые → warn.
...
### Текст и голос
- **P9 ⏸ ОТКЛЮЧЁН (Антон 2026-07-02: «наплевать на тире, ПОХЕР, забудь пока»).** Гейт тире НЕ судит и на вердикт они не влияют; кампаний по чистке старого не ведём. Правило стиля для НОВОГО моего текста остаётся (no-long-dashes [internal]) — просто пишу без них, бесплатно. Вернуть вес может только Антон.
...
Источник: Anti-Drift Review Артура Арсёнова (Ray lab, looi.ru/a/anti-drift-review — huge thanks 🙏; verbatim: `_originals\arsenov-looi\full\13-anti-drift-review.md`). Добавлено 2026-07-03 по «++» Антона. Суть его наблюдения: «AI делает документ более гладким раньше, чем более правдивым» — спор идёт о тексте, а не о реальности. Все проверки LLM-суждение по куску.
