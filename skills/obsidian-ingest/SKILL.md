...
| Dialogue subfolders (dialogues_<account>_*) | `mixed` | `hybrid` | — |
...
00. **RE-IMPORT GATE — sha256 the source BEFORE anything else (Rule 0′, set 2026-06-13 after a NO-OP catch saved a multi-hour pipeline run).** The very first action of *any* "import this corpus / add this dump / process these notes" task — **before Rule 0, before reading the file, before spawning a sub-agent** — is to ask *"did I already import this exact byte-stream?"* Run the gate: `python "$USERPROFILE\.claude\skills\obsidian-ingest\scripts\precheck_corpus.py" "<source-file>"` and read **line 1** (tab-separated `DECISION\thash\tbytes\tmatch-path`):
    ...
    Why a script and not just "remember to check": a rule that lives only in memory gets skipped under load — the gate is a deterministic Step 0 that *can't* be forgotten because it sits at the top of this list. Provenance: memory crypto-essays-reimport-idempotent [internal]. Verified live across all 4 decision paths on 2026-06-13.
...
8. **Finish with a report + link-integrity check.** End every run by validating that all `links [internal]` resolve (0 broken) and giving Anton a diff-style summary: files created, tags added, concepts mapped, duplicates resolved, anything flagged. **When the import contains `#anton-original` material, add a `candidate new concepts` line** — which emergent concepts you considered, which you created (nothing fit), and which you folded into an existing `concept-… [internal]` and why — so new-concept calls on Anton's own thinking are visible for him to confirm, never silent.
...
13. **No-orphan rule (Anton's STANDING demand, 2026-06-13 — set after he found a high-value claude-ai artifact sitting with 0 backlinks).** Every note added to the vault MUST end up with ≥1 **incoming** wikilink from an existing layer (concept / MOC / decision / parent note) — forward-links alone are not enough, the note must be discoverable by `grep` from the OTHER side. Last mandatory step of EVERY ingest, inline or batch:
    - **Orphan-check** for each new basename: `grep -lE '\[\[<basename>(\||\])' $OBSIDIAN_VAULT` → must return ≥1 file *besides* the note itself. Batch: one pass over the basename list (deterministic, 0 LLM tokens — see vault-data-architecture [internal]).
    ...
    - **Exemptions (narrow):** `_originals/` (raw archive, preserve-originals-rule [internal]), `_drafts/`, `_imports/staging/`. Live vault notes never exempt.
...
A reader (or future Anton) lands on the Cross-MOC, navigates by month/theme/person, clicks into an atomic note, sees its provenance and its concept, and follows `links [internal]` to related ideas — with zero broken links and zero duplicated raw text. The vault gets *more* navigable with every import, not just bigger.
